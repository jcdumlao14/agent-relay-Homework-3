...
Supports both SQLite (local development/Q2-Q3) and PostgreSQL (Q4+).
PostgreSQL uses normal transactional locking; task claiming adds
FOR UPDATE SKIP LOCKED in storage.py.
"""
...
RECOVERY_INTERVAL_SECONDS = max(
    1, positive_int("RELAY_RECOVERY_INTERVAL_SECONDS", 5)
)
...
def as_db_time(value: datetime) -> datetime:
    """Return UTC datetime in the format expected by the database."""

    utc_value = value.astimezone(timezone.utc)

    # SQLite DateTime is most portable with naive UTC.
    if _is_sqlite(DATABASE_URL):
        return utc_value.replace(tzinfo=None)

    # PostgreSQL with SQLAlchemy DateTime is also safe with a naive UTC value.
    # Keeping this normalized makes comparisons consistent across both DBs.
    return utc_value.replace(tzinfo=None)
...
class Task(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "sender_id",
            "idempotency_key",
            name="uq_task_sender_idempotency",
        ),
    )
...
class Attempt(Base):
    ...
    terminal_action: Mapped[str | None] = mapped_column(
        String(10),
        nullable=True,
    )

    terminal_payload_hash: Mapped[str | None] = mapped_column(
        String(64),
        nullable=True,
    )
...
def immediate_transaction() -> Generator[Session, None, None]:
    ...
    SQLite:
        Uses BEGIN IMMEDIATE to reserve the writer transaction.

    PostgreSQL:
        Uses the normal SQLAlchemy transaction. Individual row-level
        locking is applied by storage.claim_one() with
    ...
    try:
        ...
        elif _is_postgresql(DATABASE_URL):
...
def recover_expired_in_session(
    db: Session,
    now: datetime,
) -> int:
    """Expire active leases and requeue/fail their tasks."""
...
def recover_expired() -> int:
    ...
    with immediate_transaction() as db:
        return recover_expired_in_session(
            db,
            utcnow(),
        )
