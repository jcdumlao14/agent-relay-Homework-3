...
SQLite uses the existing BEGIN IMMEDIATE transaction mechanism.

PostgreSQL uses row-level locking with FOR UPDATE SKIP LOCKED when
claiming queued tasks so concurrent workers do not claim the same task.
"""
...
def authenticate(token: str) -> Agent:
    ...
    # Use the same transaction boundary as task operations.
    with immediate_transaction() as db:
        agent = db.scalar(
            select(Agent).where(
                Agent.token_hash == token_digest
            )
        )
...
def create_task(
    ...
    # Serializing task creation makes the sender-scoped idempotency check
    # and unique constraint one operation even when two API processes race.
    with immediate_transaction() as db:
...
def claim_one(
    ...
    with immediate_transaction() as db:
        ...
        # Recover expired claims before selecting new work.
        recover_expired_in_session(
            db,
            now,
        )

        # Build the queued-task query.
        query = (
            select(Task)
            .where(
        ...
        # PostgreSQL:
        # Lock the selected row and skip rows already locked by another
        # worker. This prevents concurrent workers from claiming the same
        # queued task.
        #
        # SQLite:
        # Do not use FOR UPDATE because SQLite does not support it.
        # BEGIN IMMEDIATE in database.py provides the SQLite transaction
...
def heartbeat(
    ...
    with immediate_transaction() as db:
        ...
        attempt = _find_attempt_for_token(
            db,
            task_id,
            claim_token,
        )
        ...
        attempt.lease_expires_at = as_db_time(
            now + timedelta(
                seconds=LEASE_SECONDS
            )
        )
    ...
    with immediate_transaction() as db:
        ...
        if attempt.outcome in {
            ...
            if (
                ...
                and attempt.terminal_payload_hash
                == value_digest
            ):
                return {
                    "task_id": task.id,
            ...
            raise RelayError(
                ...
                "A different terminal result was already accepted.",
                409,
            )
        ...
        if (
            db_time(attempt.lease_expires_at) is None
            or db_time(attempt.lease_expires_at) <= now
        ):
            raise RelayError(
                "stale_claim",
...
def task_for_participant(
    ...
    with db_session() as db:
        ...
        if (
            task is None
            or (
                task.sender_id != agent_id
                and task.recipient_id != agent_id
            )
        ):
            raise RelayError(
                "not_found",
...
def list_tasks(
    ...
    with db_session() as db:
        ...
        owner_column = (
            Task.sender_id
            if direction == "sent"
            else Task.recipient_id
        )

        query = (
            select(Task)
            .where(owner_column == agent_id)
            .order_by(
                Task.created_at,
                Task.id,
            )
...
def encode_cursor(
    ...
    # Response timestamps intentionally use second precision,
    # but a cursor must retain microseconds or several rows created
    # in one second can repeat on the next page.
